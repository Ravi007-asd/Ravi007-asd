...
# Streamlit UI
st.set_page_config(page_title="Ravi Teja Stock Predictor", layout="wide")
st.title("📈 Ravi Teja Stock Predictor")

# Initialize session state for user holdings if not already set
if 'balance' not in st.session_state:
    st.session_state.balance = 10000  # Default balance
...
st.subheader("📊 Ravi Teja Stock Performance")
st.write(df[['Date', 'Open', 'Close', 'Change']].tail(7))

# Display user's holdings dynamically
st.subheader("📌 Ravi Teja User Holdings")
